...
public final class MapPalette {
    ...
    public static byte matchColor(@NotNull Color color, boolean useCache) {
        if (color.getAlpha() < 128) return 0;

        if (useCache && mapColorCache != null && mapColorCache.isCached()) {
    ...
    public static MapColorCache mapColorCache;
    ...
    public static void setMapColorCache(@NotNull MapColorCache mapColorCache) {
        ...
        try {
            ((CraftMapColorCache) mapColorCache).initCache().get();
        } catch (Exception e) {
            e.printStackTrace();
        }
